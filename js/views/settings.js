...
KM.views.settings = {
  render: function () {
    ...
    return '<div class="page narrow"><div class="page-head"><h1>Настройки</h1></div>' +
      ...
      '<section class="card stack mt"><h2>🤖 AI-ассистент (свой ключ)</h2><p class="small mb0">Если на сайте AI не настроен, ассистент может работать напрямую из браузера с вашим ключом Anthropic (<code>sk-ant-…</code>, console.anthropic.com) или OpenRouter (<code>sk-or-…</code>, openrouter.ai/keys). Ключ хранится только в этом браузере и отправляется только на api.anthropic.com или openrouter.ai. Используйте ключ с лимитом расходов и не вводите его на чужих устройствах.</p>' +
        '<div class="row"><input type="password" id="aikey" placeholder="sk-ant-… или sk-or-…" value="' + KM.esc(s.aiKey || '') + '" style="max-width:360px" autocomplete="off"><button class="btn" id="aisave">Сохранить</button><button class="btn ghost" id="aidel">Удалить</button></div></section>' +
      '</div>';
  },
  mount: function (root) {
    var s = KM.store.state.settings;
